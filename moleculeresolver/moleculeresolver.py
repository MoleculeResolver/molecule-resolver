...
class MoleculeResolver:
    ...
    empirical_formula_regex_compiled = regex.compile(r'([A-IK-Z][a-ik-z]*)([0-9]+(?:[.][0-9]+)?)?')
    formula_bracket_group_regex_compiled = regex.compile(r'(\((?:[^()]|(?R))*\))(\d+(?:\.\d+)?)')
    non_generic_SMILES_regex_compiled = regex.compile(r'^[a-ik-zA-IK-Z0-9%=#$@+\-\[\]\(\)\\\/\:\.]*$') # non-generic SMILES, no wildcards or unspecified bonds
    InChI_regex_compiled = regex.compile(r'^InChI=\dS?\/[0-9a-ik-zA-IK-Z]+\/[0-9a-ik-zA-IK-Z+\-\(\)\\\/,\?]*$')
    ...
    def __init__(self, available_service_API_keys: Optional[dict[str, Optional[str]]] = None, molecule_cache_db_path: Optional[str] = None, molecule_cache_expiration: Optional[datetime] = None, differentiate_isotopes: bool = False):
    ...
    def __enter__(self):
        ...
        self.molecule_cache = SqliteMoleculeCache(self.molecule_cache_db_path, self.molecule_cache_expiration)
        self.molecule_cache.__enter__()
        if 'opsin' in self._available_services_with_batch_capabilities:
            self._OPSIN_tempfolder = tempfile.TemporaryDirectory(prefix='OPSIN_tempfolder_')
        return self
    ...
    @contextmanager
    def query_molecule_cache(self, service: str, identifier_mode: str, identifier: str) -> Tuple[bool, list[Molecule]]:
    ...
    def _init_session(self, pool_connections: Optional[int] = None, pool_maxsize: Optional[int] = None) -> None:
        ...
        self._session.mount('https://', requests.adapters.HTTPAdapter(pool_connections=pool_connections, pool_maxsize=pool_maxsize, max_retries=2))
    ...
    def _resilient_request(self, url: str, kwargs: Optional[dict[str, Any]] = None, request_type: str = 'get', accepted_status_codes: list[int]=[200], rejected_status_codes: list[int] = [404], max_retries: int = 10, sleep_time: Union[int, float] = 2, allow_redirects: bool = False, json:Any=None, return_response:bool=False) -> Optional[requests.Response]:
        ...
        session = self._session if 'comptox' not in url.lower() else self._session_CompTox
        n_try = 0
        while True:
            n_try += 1
            try:
                ...
                if request_type == 'get':
                    response = session.get(url, **kwargs, json=json, allow_redirects=allow_redirects)
                ...
                if response.status_code in accepted_status_codes:
                    ...
                    if response_text != response.text:
                        if response_text.count(r'\u') + response_text.count(r'\x') > 0:
                            raise UnicodeEncodeError('Wrong charachter encoding was used.')
    ...
    @staticmethod
    @cache
    def standardize_SMILES(SMILES: str, standardize: bool, /, remove_atom_mapping_number: bool = True, disconnect_metals: bool = False, normalize: bool = True, reionize: bool = True, uncharge: bool = False, stereo: bool = True, canonicalize_tautomer: bool = False, isomeric_SMILES: bool = True) -> str:
    ...
    def standardize_molecule(mol: Chem.rdchem.Mol, /, remove_atom_mapping_number: bool = True, disconnect_metals: bool = False, normalize: bool = True, reionize: bool = True, uncharge: bool = False, stereo: bool = True, canonicalize_tautomer: bool = False) -> Optional[Chem.rdchem.Mol]:
        ...
        if len(mol_fragments) > 1:
            ...
            for mol_fragment in mol_fragments:
                standardized_mol_fragment = MoleculeResolver.standardize_molecule(mol_fragment,
        ...
        if reionize:
            ...
            try:
                ...
                warnings.warn('Reionization step could not be performed. It will be skipped.')
        ...
        if canonicalize_tautomer:
            mol = MoleculeResolver.get_from_SMILES(canonicalize_tautomer_smiles(Chem.MolToSmiles(mol)))
    ...
    @staticmethod
    def _check_and_flatten_identifiers_and_modes(identifiers: Union[str, list[str], list[list[str]]], modes: Union[str, list[str]]) -> tuple[list[str], list[str], list[str], list[str], str]:
        ...
        if not len(given_SMILES) < 2:
            raise ValueError('Only one valid structure is accepted to search for (this includes SMILES and InChI).')
        if len(given_SMILES) > 0:
            given_SMILES = given_SMILES[0]
        else:
            given_SMILES = None

        MoleculeResolver._check_parameters(modes = flattened_modes, identifiers = flattened_identifiers, context = 'find_single')
    ...
    @staticmethod
    def _check_parameters(*, modes = None, services = None, identifiers = None, required_formulas = None, required_charges = None, required_structure_types = None, context = 'get_molecule'):
        ...
        if modes is not None:
            if context == 'get_molecule' or context == 'get_molecules_batch':
                ...
                if services is not None and modes not in MoleculeResolver._supported_modes_by_services[services]:
                    raise ValueError(f'The chosen mode ({modes}) is not compatible with the service {services}.')
                
            if context == 'batch' or context == 'find_single':
                if not isinstance(modes, list):
                    raise TypeError('The modes parameter can only be a list of strings.')
                if not all([isinstance(temp_mode, str) for temp_mode in modes]):
                    if not MoleculeResolver._is_list_of_list_of_str(modes):
                        raise TypeError('The modes parameter can only be a list of strings.')
                if identifiers is not None and isinstance(identifiers, list) and not len(modes) == len(identifiers): # you can give several identifiers e.g. [['sodium chloride', 'NaCl']] with modes = ['name']
                    raise ValueError('The modes and identifiers parameters must be of the same length.')
                
            if context == 'find_multiple' and not all([len(mode) > 0 for mode in modes]):
                raise ValueError('The list of modes cannot include an empty string or an empty list.')
        ...
        if services is not None:
            ...
            if context == 'batch' or context == 'find_single':
                if not isinstance(services, list):
                    raise TypeError('The services parameter can only be a list of strings.')
                if not all([isinstance(temp_service, str) for temp_service in services]):
                    raise TypeError('The services parameter can only be a list of strings.')
                if not all([temp_service in MoleculeResolver._available_services for temp_service in services]):
                    raise ValueError('The list of services can only include supported services.')

        if required_formulas is not None:
            if context == 'find_multiple' and identifiers is not None and not len(identifiers) == len(required_formulas):
                raise ValueError('The identifiers and required_formulas parameters must be of the same length.')
        ...
        if required_charges is not None:
            if context == 'get_molecule':
                if not isinstance(required_charges, str) and not isinstance(required_charges, int):
                    raise TypeError('required_charge can be zero, not_zero, positive, negative or any integer.')
                if isinstance(required_charges, str):
                    if required_charges not in ['zero', 'non_zero', 'positive', 'negative']:
                        raise ValueError("required_charge can be 'zero', 'non_zero', 'positive', 'negative' or any integer.")

            if context == 'find_multiple':
                if not isinstance(required_charges, list):
                    raise TypeError('The required_charges must be a list.')
                if identifiers is not None and not len(identifiers) == len(required_charges):
                    raise ValueError('The parameters identifiers and required_charges must be of the same length.')

        if required_structure_types is not None:
            if context == 'find_multiple' and identifiers is not None and not len(identifiers) == len(required_structure_types):
                raise ValueError('The parameters identifiers and required_structure_types must be of the same length.')
        
            if context == 'get_molecule':
                if not isinstance(required_structure_types, str):
                    raise TypeError('required_structure_type must be a string.')
                if required_structure_types not in ['mixture_neutrals','mixture_ions', 'neutral', 'salt', 'ion','mixture_neutrals_salts', 'mixture_neutrals_ions']:
                    raise ValueError("required_structure_type must be one of the following: 'mixture_neutrals','mixture_ions', 'neutral', 'salt', 'ion','mixture_neutrals_salts', 'mixture_neutrals_ions'")

    @staticmethod
    def take_most_common(container: list[Any], number_to_take: Optional[int] = None) -> list[Any]:
        ...
        sorted_item_indices_to_take = [item_index for item_index, _ in counter.most_common(number_to_take)]
    ...
    def filter_and_sort_synonyms(synonyms : list[str], number_of_synonyms_to_take: int = 5, strict: bool = False) -> list[str]:
        ...
        for synonym in temp:
            ...
            if synonym is not None:
                ...
                tokens = ['SCHEMBL', 'AKOS', 'MFCD', 'AKOS', 'CYPHOS', 'DTXSID', 'UNII', 'NSC', 'DSSTox', 'FLUKA', 'ACMC', '%', '(at)'\
                        ,'SIGMA', 'ALDRICH', 'RIEDEL', 'SIAL']
                ...
                ratio_uppercase_letters = sum(1 for c in synonym if c.isupper()) / len(synonym)

                if ratio_uppercase_letters < 0.5:
                    match = regex.match(MoleculeResolver.CAS_regex_with_groups, synonym) # filter CAS numbers
                    # filter names with 3 consecutive numbers or containing some keywords
                    match2 = regex.search(r'\d{3}|\bin\b|\bgrade\b|\bsolution\b|\bstandard\b|\bstabilized\b|\bdimer\b|\bfor\b|\btablet\b|\btotal\b|\bcode\b|\bNo\.\b|\banhydrous\b', synonym, regex.IGNORECASE) 
                    if not match and not match2:
                        synonym = synonym.replace('<em>', '').replace('</em>', '') #replace(';', ', ')
        ...
        final_synonyms_taken = MoleculeResolver.take_most_common(synonyms_taken, number_of_synonyms_to_take)
    ...
    def expand_name_heuristically(name: str,
        ...
        for part in parts_to_delete:
            for temporary_name in names:
                new_name = regex.sub(rf'([;, ]*\(?\s*{part}\s*\)?\s*-?)', '', temporary_name)
                if new_name not in names:
                    names.append(new_name)
        ...
        for temporary_name in names:
            for (pattern, replacement) in maps_to_replace:
                ...
                if ms:
                    if all([v not in ms[0] for v in ['trans', 'cis', 'alpha', 'beta', 'tert', 'sec', 'gamma', 'erythro', 'threo']]):
    ...
    def combine_molecules(molecules: list[Molecule], return_None_when_different_SMILES:bool = False) -> Molecule:
        ...
        all_SMILES_are_equal = all([MoleculeResolver.are_equal(MoleculeResolver.get_from_SMILES(cmp.SMILES), MoleculeResolver.get_from_SMILES(molecules[0].SMILES)) for cmp in molecules])
        all_identifiers_are_equal = all([cmp.identifier == molecules[0].identifier for cmp in molecules])
        ...
        if all_molecules_used_same_service:
            merged_additional_information = [v for v in merged_additional_information if v is not None]
            if merged_additional_information:
                merged_additional_information = str(sorted(merged_additional_information))
            else:
                merged_additional_information = ''
        ...
        return Molecule(molecules[0].SMILES, merged_synonyms, merged_CAS, merged_additional_information, merged_modes, merged_services, number_of_crosschecks, molecules[0].identifier)

    @staticmethod
    def group_molecules_by_structure(molecules: list[Molecule], group_also_by_services: bool = True) -> dict[str, list[Molecule]]:
        ...
        all_molecules_from_primary_source = all([cmp.service in MoleculeResolver._available_services and cmp.number_of_crosschecks == 1 for cmp in molecules])
        if all_molecules_from_primary_source:
            ...
            for cmp in molecules:
                ...
                for key, grouped_molecule_mol in grouped_molecules_molecules.items():
                    if MoleculeResolver.are_equal(cmp_mol, grouped_molecule_mol):
                        ...
                        equal_SMILES = sorted([equal_SMILES, grouped_molecules_SMILES[key]])[0]
                        old_key = key
                        break
                ...
                if key not in grouped_molecules:
                    ...
                    grouped_molecules_molecules[key] = MoleculeResolver.get_from_SMILES(equal_SMILES)
            ...
            if len(grouped_molecules) > 1:
                all_molecules_used_same_service_and_mode = all([cmp.mode == molecules[0].mode and cmp.service == molecules[0].service for cmp in molecules])
            ...
            grouped_molecules = {grouped_molecules_SMILES[key]: grouped_molecules[key] for key in all_keys}
        else:
            raise ValueError('This function only takes molecules that were not previously combined from different sources.')

        return grouped_molecules

    @staticmethod
    def filter_molecules(molecules: list[Molecule], required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = False) -> list[Molecule]:
        filtered_molecules = []
    ...
    @staticmethod
    def filter_and_combine_molecules(molecules: Union[list[Molecule], Molecule], required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = False) -> Optional[Molecule]:

        filtered_molecules = MoleculeResolver.filter_molecules(molecules, required_formula, required_charge, required_structure_type, standardize)

        if not filtered_molecules:
            return None

        grouped_molecules = MoleculeResolver.group_molecules_by_structure(filtered_molecules)
    ...
    @staticmethod
    def formula_to_dictionary(formula: str, allow_duplicate_atoms: bool = True) -> dict[str, Union[int, float]]:

        def merge_dictionaries(d1: dict[str, Union[int, float]], d2: dict[str, Union[int, float]], d2_multiplier: float = 1.0):
            d1 = d1.copy()
        ...
        formula = formula.replace(']', ')').replace('}', ')')
        brackets_match = MoleculeResolver.formula_bracket_group_regex_compiled.search(formula)
        if brackets_match:
            rest_of_formula = formula[0:brackets_match.span()[0]] + formula[brackets_match.span()[1]:]
            ...
            d = merge_dictionaries(
                    MoleculeResolver.formula_to_dictionary(rest_of_formula, allow_duplicate_atoms), 
                    MoleculeResolver.formula_to_dictionary(bracket_content, allow_duplicate_atoms), 
                    bracket_multiplier
                )
        else:
    ...
    @staticmethod
    def check_formula(mol_or_formula: Union[Chem.rdchem.Mol, str], required_formula: Optional[str]) -> bool:
    ...
    def check_structure_type(SMILES: str, required_structure_type: str) -> bool:
        ...
        available_structure_types = ['mixture_neutrals','mixture_ions', 'neutral', 'salt', 'ion','mixture_neutrals_salts', 'mixture_neutrals_ions']
        if required_structure_type not in available_structure_types:
            raise ValueError("required_structure_type must be one of 'mixture_neutrals','mixture_ions', 'neutral', 'salt', 'ion','mixture_neutrals_salts' or 'mixture_neutrals_ions'.")

        return MoleculeResolver.get_structure_type_from_SMILES(SMILES) == required_structure_type
    ...
    def get_from_SMILES(SMILES: str, addHs: bool = False, with_3D_structure: Optional[str] = None) -> Chem.rdchem.Mol:
        ...
        if mol is None:
            ...
            if SMILES.count('F') > 0 or SMILES.count('Cl') > 0 or SMILES.count('Br') > 0 or SMILES.count('B') > 0:
    ...
    @staticmethod
    def show_and_pause(mol: Chem.rdchem.Mol, name: Optional[str] = None, size: tuple[int, int] = (1000,1000)):
    ...
    @staticmethod
    def to_hill_formula(mol_or_dictionary: Union[Chem.rdchem.Mol, dict[str, Union[int, float]]]) -> str:
    ...
    def are_equal(mol1: Chem.rdchem.Mol, mol2: Chem.rdchem.Mol, standardize: bool = True, isomeric: bool = True, check_for_resonance_structures: Optional[bool] = None, method: int = 1) -> bool:
        ...
        if method == 1:
            ...
            if check_for_resonance_structures:
                ...
                return len(unique_partial_SMILES1) == len(matching_unique_partial_SMILES2_found)
        ...
        if method == 2: # using the RDKit RegistrationHash
            ...
            def get_mol_hash(mol, hashscheme):
                return RegistrationHash.GetMolHash(RegistrationHash.GetMolLayers(mol), hash_scheme = hashscheme)
    ...
    @staticmethod
    def find_duplicates_in_molecule_dictionary(molecules: dict[Any, Molecule], clustered_molecules = None):
                
        if not clustered_molecules:
            clustered_molecules = MoleculeResolver.group_molecule_dictionary_by_formula(molecules)

        yield from MoleculeResolver.intersect_molecule_dictionaries(molecules, molecules, clustered_molecules, clustered_molecules, report_same_keys = False)

    @staticmethod
    def group_molecule_dictionary_by_formula(molecules: dict[Any, Molecule]) -> dict[str, dict[Any, Tuple[Chem.rdchem.Mol, Molecule]]]:
        temporary = {}
    ...
    def intersect_molecule_dictionaries(molecules: dict[Any, Molecule], other_molecules: dict[Any, Molecule], mode: str = 'SMILES', 
        ...
        if not clustered_other_molecules:
            clustered_other_molecules = MoleculeResolver.group_molecule_dictionary_by_formula(other_molecules)
    ...
    def get_SMILES_from_Mol_format(self, *, molblock: Optional[str] = None, url: Optional[str] = None, standardize: bool = True) -> Optional[str]:
        ...
        def fix_MolBlock(mol_block):
            ...
            while change_was_made:
                ...
                three_digit_blocks = [count_line[i:i+3] for i in range(0, len(count_line), 3)]

                for i_block, three_digit_block in enumerate(three_digit_blocks):
                    if three_digit_block != '   ' and three_digit_block[-1] == ' ':
                        count_line = count_line[:i_block * 3] + ' ' + count_line[i_block * 3:]
                        change_was_made = True
                        break
    ...
    def get_SMILES_from_image_file(self, image_path: str, engines_order: list[str] = ['osra', 'molvec', 'imago'], mode: str = 'single', standardize: bool = True) -> str:
        ...
        with open(image_path, 'rb') as handle:
            try:
                response_text = self._resilient_request('https://molvec.ncats.io/all', {'data': handle.read()}, request_type='post')

                data = json.loads(response_text)
                for engine in engines_order:
                    if data[engine]['status'].lower() in  ['success', 'cached']:
                        temp_SMILES = self.get_SMILES_from_Mol_format(molblock=data[engine]['molfile'])
    ...
    @staticmethod
    def save_to_PNG(mol: Chem.rdchem.Mol, filename: str, atom_infos = None, atom_infos_format_string: str = '%.3f', size: set[int, int] = (1000,1000)):
    ...
    @staticmethod
    def parse_items_from_html(html_: str, split_tag: str, properties_regex: list[Tuple[str, str, list[int]]], property_indices_required: Optional[list[int]] = None):
            ...
            for (source_type, property_regex, allowed_match_number) in properties_regex:
                ...
                if allowed_match_number:
                    if len(property_matches) not in allowed_match_number:
                        raise RuntimeError('The webpage either changed its format or the regex is picking up something unwanted.')
                value = property_matches[0].strip() if property_matches else None
                item.append(value)

            add_item = False
            if property_indices_required:
                add_item = all([item[property_index] is not None for property_index in property_indices_required])
    ...
    def get_molecule_from_OPSIN(self, name: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True, allow_warnings: bool = False) -> Optional[Molecule]:
        ...
        MoleculeResolver._check_parameters(required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, services='opsin')

        with self.query_molecule_cache('opsin', 'name', name) as (entry_available, molecules):
            if not entry_available:
                response_text = self._resilient_request(f'https://opsin.ch.cam.ac.uk/opsin/{urllib.parse.quote(name)}')

                if response_text is not None:
                    temp = json.loads(response_text)
                    if temp['status'] == 'SUCCESS' or (allow_warnings and temp['status']== 'WARNING'):
                        if MoleculeResolver.check_SMILES(temp['smiles'], required_formula, required_charge, required_structure_type):
                            SMILES = temp['smiles']
                            SMILES = MoleculeResolver.standardize_SMILES(SMILES, standardize)
                        else:
                            SMILES_from_InChI = MoleculeResolver.InChI_to_SMILES(temp['inchi'], standardize)
                            if MoleculeResolver.check_SMILES(SMILES_from_InChI, required_formula, required_charge, required_structure_type):
                                SMILES = MoleculeResolver.standardize_SMILES(SMILES_from_InChI, standardize)
    ...
    def get_molecule_from_OPSIN_batchmode(self, names: list[str], standardize: bool = True) -> list[Optional[Molecule]]:

        MoleculeResolver._check_parameters(identifiers=names, modes='name', services='opsin', context='get_molecules_batch')

        if not self._java_path:
            raise FileNotFoundError('The java installation could not be found. Either it is not installed or its location has not been added to the path environment variable.')
        ...
        def download_OPSIN_offline_and_convert_names(names: list[str], tempfolder: str):
            ...
            if not os.path.exists(os.path.join(tempfolder, 'opsin.jar')):
                response_text = self._resilient_request('https://api.github.com/repos/dan2097/opsin/releases/latest')
                temp = json.loads(response_text)
            ...
            _ = subprocess.run([self._java_path, '-jar', 'opsin.jar', '-osmi', f'input_{unique_id}.txt', f'output_{unique_id}.txt'], capture_output=True, cwd=tempfolder)        
    ...
    def get_molecule_for_ion_from_partial_pubchem_search(self, identifier: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, standardize: bool = True) -> Optional[list]:
            ...
            for cid, in items:
                ...
                if is_salt_or_mixture:
                    ...
                    for temptative_SMILES in SMILES_list:
                        ...
                        if MoleculeResolver.check_formula(temp_mol, required_formula) and MoleculeResolver.check_charge(temp_mol, required_charge):
        ...
        return [(Molecule(smi, service='pubchem'), y) for smi, y in counts.most_common()]

    def get_molecule_from_ChEBI(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        if required_formula is None:
            if mode == 'formula':
                required_formula = identifier
        
        MoleculeResolver._check_parameters(services='chebi', modes=mode, required_charges=required_charge, required_structure_types=required_structure_type, required_formulas=required_formula)

        with self.query_molecule_cache('chebi', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                ...
                if search_response_text is not None:
                    ...
                    temp = root['S:Envelope']['S:Body']['getLiteEntityResponse']['return']
                    if not temp:
                        return None
                    ...
                    for ChEBI_id in [item['chebiId'] for item in temp_list]:
                        ...
                        molecule_response_text = self._resilient_request(f'{CHEBI_URL}getCompleteEntity?chebiId={ChEBI_id}')

                        if molecule_response_text is not None:
                            root = xmltodict.parse(molecule_response_text)
                            molecule = root['S:Envelope']['S:Body']['getCompleteEntityResponse']['return']
                            ...
                            if 'IupacNames' in molecule:
                                if isinstance(molecule['IupacNames'], list):
                                    synonyms.extend([synonym['data'] for synonym in molecule['IupacNames']])
                            ...
                            if 'RegistryNumbers' in molecule:
                                ...
                                if not isinstance(molecule['RegistryNumbers'], list):
                                    temp = [molecule['RegistryNumbers']]
                                CAS = MoleculeResolver.filter_and_sort_CAS([synonym['data'] for synonym in temp])

                            molecule = Molecule(SMILES, synonyms, CAS, ChEBI_id.split(':')[1], mode, service='chebi')
                            molecules.append(molecule)

        return MoleculeResolver.filter_and_combine_molecules(molecules, required_formula, required_charge, required_structure_type, standardize)

    def get_molecules_using_batchmode_from(self, identifiers: str, modes: str, service: str, batch_size: int = 1000, progressbar: bool = False, ignore_exceptions: bool = False) -> tuple[dict[str, list[Optional[Molecule]]], list[str]]:

        if service not in self._available_services_with_batch_capabilities:
            raise ValueError(f'The service {service} does not have batch capabilities or is not supported.')
        ...
        for _modes in modes:
            ...
            [all_supported_modes.append(mode) for mode in _modes if mode not in all_supported_modes and mode in MoleculeResolver._supported_modes_by_services[service]]

        modes = new_modes

        MoleculeResolver._check_parameters(identifiers=identifiers, modes=modes, context='batch')
        ...
        for i_molecule, (i_molecule_identifiers, i_molecule_modes) in enumerate(zip(identifiers, modes)):
            if len(i_molecule_identifiers) != len(i_molecule_modes):
                raise ValueError('The list of identifiers and modes of a molecule must be of the same length.')
        ...
        for mode in all_supported_modes:
            ...
            chunks = list(MoleculeResolver.chunker(list(zip(_identifiers, _identifier_indices)), batch_size))
            ...
            if chunks:
                for chunk in tqdm(chunks, disable=not progressbar):
                    ...
                    if service == 'pubchem':
                        results_for_this_chunk = self.get_molecules_from_pubchem_batchmode(identifier_chunk, mode, True)
                    elif service == 'srs':
                        results_for_this_chunk = self.get_molecules_from_SRS_batchmode(identifier_chunk, mode, True)
                    elif service == 'comptox':
                        results_for_this_chunk = self.get_molecules_from_CompTox_batchmode(identifier_chunk, mode, True)
                    elif service == 'opsin':
                        results_for_this_chunk = self.get_molecule_from_OPSIN_batchmode(identifier_chunk, True)
                    else:
                        raise NotImplementedError('The batch functionality for this service is either not implemented or the service does not support batch searches.')

                    for index, result in zip(identifier_indices_chunk, results_for_this_chunk, strict=True):
                    ...
                    if len(set(identifier_indices_chunk) - set(this_mode_results.keys())) != 0:
                        raise RuntimeError('The number of returned values should be equal to the unique values asked for.')
    ...
    def get_molecule_from_CompTox(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        # new API, at some point we need to change
        # https://api-ccte.epa.gov/docs/chemical.html#/

        MoleculeResolver._check_parameters(modes=mode, required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, services='comptox')

        with self.query_molecule_cache('comptox', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                        ...
                        if substance_response_text is not None:
                            ...
                            if 'preferredName' in temp_substance:
                                if temp_substance['preferredName'] is not None:
                                    temp_synonyms.append(temp_substance['preferredName'])
                            ...
                            if 'wikipediaName' in temp_substance:
                                if temp_substance['wikipediaName'] is not None:
                                    temp_synonyms.append(temp_substance['wikipediaName'])
    ...
    def get_molecule_from_CTS(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:

        MoleculeResolver._check_parameters(identifiers=identifier, modes=mode, required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, services='cts')

        with self.query_molecule_cache('cts', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                cts_modes = {
                    'name': 'Chemical Name',
                    'cas': 'CAS',
                ...
                if SMILES is not None:
                    molecules.append(Molecule(SMILES, synonyms, CAS, None, mode_used, 'cts'))
    ...
    def get_CompTox_request_unique_id() -> str:
        ...
        def base36encode(number, alphabet='0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'.lower()):
    ...
    def get_molecules_from_CompTox_batchmode(self, identifiers: list[str], mode: str, standardize: bool = True) -> list[Optional[Molecule]]:
        MoleculeResolver._check_parameters(identifiers=identifiers, modes=mode, services='comptox', context='get_molecules_batch')
        ...
        with self.query_molecule_cache_batchmode('comptox', mode, identifiers) as (identifiers_to_search, indices_of_identifiers_to_search, results):
            ...
            postdata = {
                'downloadItems': ['SYNONYM_IDENTIFIER','QC_LEVEL','CASRN', 'IUPAC_NAME', 'SMILES'],
                'downloadType' : 'EXCEL',
    ...
    def get_molecule_from_Chemeo(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        ...
        with self.query_molecule_cache('chemeo', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                valid_token_found = self.available_service_API_keys['chemeo'] is not None
                if valid_token_found:
                    valid_token_found = isinstance(self.available_service_API_keys['chemeo'], str)
                    valid_token_found = MoleculeResolver.chemeo_API_token_regex_compiled.match(self.available_service_API_keys['chemeo'])
                ...
                if not valid_token_found:
                    if 'chemeo_API_token_missing' not in self._message_slugs_shown:
                        ...
                        warnings.warn('chemeo requires a valid API token to allow search. After registering please insert the token in the corresponding variable when initializing the class instance.')
                ...
                if request_text is not None and request_text != '{"message":"Compound not found."}':
                    temp = json.loads(request_text)
                    request_text = self._resilient_request(f'{CHEMEO_URL}search?q={temp["cid"]}',
                                                kwargs={'headers' : API_bearer_headers}, 
                                                rejected_status_codes=[403, 404])
                    ...
                    for i in range(temp['total']):
                        ...
                        if 'mol3d' in result:
                            temp_SMILES = self.get_SMILES_from_Mol_format(molblock=result['mol3d'], standardize=standardize)

                        if temp_SMILES is None and 'mol2d' in result:
                            temp_SMILES = self.get_SMILES_from_Mol_format(molblock=result['mol2d'], standardize=standardize)

                        if temp_SMILES is None and 'inchi' in result:
                            temp_SMILES = MoleculeResolver.InChI_to_SMILES(result['inchi'], standardize)
                        ...
                        molecules.append(Molecule(temp_SMILES, temp_synonyms, temp_CAS, result['id'], mode, 'chemeo'))

        return MoleculeResolver.filter_and_combine_molecules(molecules, required_formula, required_charge, required_structure_type, standardize)

    def get_molecules_from_pubchem_batchmode(self, original_identifiers: list[str], mode: str, standardize: bool =True) -> list[Optional[list[Molecule]]]:
        # https://pubchem.ncbi.nlm.nih.gov/docs/power-user-gateway
        # https://pubchem.ncbi.nlm.nih.gov/docs/identifier-exchange-service
        MoleculeResolver._check_parameters(identifiers=original_identifiers, modes=mode, services='pubchem', context='get_molecules_batch')
    ...
    def get_molecule_from_pubchem(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        ...
        MoleculeResolver._check_parameters(identifiers=identifier, required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, modes=mode, services='pubchem')
        
        with self.query_molecule_cache('pubchem', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                SMILES = None
                synonyms = []
                CAS = []
                ...
                if request_text is not None:
                    ...
                    if pubchem_mode == 'formula':
                        ...
                        cids_response_text = self._resilient_request(f'{PUBCHEM_URL}listkey/{listkey}/cids/json',
                                                                rejected_status_codes=[400, 404], accepted_status_codes=[200], sleep_time=7)

                        if cids_response_text is not None:

                            cid_list = json.loads(cids_response_text)['IdentifierList']['CID']

                            for cid_temp in cid_list:
                                cmp_temp = self.get_molecule_from_pubchem(str(cid_temp), 'cid', required_formula, required_charge, required_structure_type, standardize)
                                if cmp_temp:
                                    cmp_temp.mode = mode
                        ...
                        # if nothing was foung, try with the hill formula, as pubchem sometimes finds results using it
                            hill_formula = MoleculeResolver.to_hill_formula(MoleculeResolver.formula_to_dictionary(identifier))
                            if identifier != hill_formula:
                                return self.get_molecule_from_pubchem(hill_formula, mode, required_formula, required_charge, required_structure_type, standardize)

                    else:

                        def get_prop_value(_compound, label, name, conversion_funtion, all_names=False):
                            props_found = [prop['value'] for prop in _compound['props'] if prop['urn']['label'] == label and prop['urn']['name'] == name]

                            if all_names:
                                return {prop['urn']['name']: conversion_funtion(prop['value']['sval']) for prop in _compound['props'] if prop['urn']['label'] == label}
                            else:
    ...
    def get_molecule_from_CAS_registry(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        MoleculeResolver._check_parameters(required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, modes=mode, services='cas_registry')

        with self.query_molecule_cache('cas_registry', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                ...
                if search_response_text is not None:
                    ...
                    for result in results:
                        CAS = result['rn']
                        detail_response_text = self._resilient_request(f'{CAS_URL}detail?cas_rn={urllib.parse.quote(CAS)}',
                        ...
                        if detail_response_text is not None:
                            ...
                            if MoleculeResolver.check_SMILES(SMILES, required_formula, required_charge, required_structure_type):
                                molecules.append(Molecule(SMILES, MoleculeResolver.filter_and_sort_synonyms(synonyms), [CAS], None, mode_used, 'cas_registry'))
                            elif inchi != '':
                                SMILES_from_InChI = MoleculeResolver.InChI_to_SMILES(inchi, standardize)
                                if MoleculeResolver.check_SMILES(SMILES_from_InChI, required_formula, required_charge, required_structure_type):
                                    molecules.append(Molecule(SMILES_from_InChI, MoleculeResolver.filter_and_sort_synonyms(synonyms), [CAS], None, mode_used, 'cas_registry'))
                ...
                if mode == 'smiles' and len(molecules) == 0:
                    ...
                    cmp = self.get_molecule_from_CAS_registry(inchi, 'inchi', required_formula, required_charge, required_structure_type, standardize)
                    if cmp is not None: cmp.mode = 'inchi calculated from smiles'
                    molecules.append(cmp)

        return MoleculeResolver.filter_and_combine_molecules(molecules, required_formula, required_charge, required_structure_type, standardize)

    def _match_SRS_results_to_identifiers(self, identifiers: list[str], mode: str, standardize: bool, results: list[dict]):
        ...
        for result in results:
            ...
            if ITN in infos_by_ITN:
                raise RuntimeError('ITN already exists in infos_by_ITN, ITNs are expected to be returned only once.')
                ...
                if identifier not in SRS_results_by_identifier:
                    ...
                    if ITNs_for_this_identifier:
                        if len(ITNs_for_this_identifier) == 1:
                            ...
                            temptative_ITNs = set(ITNs_for_this_identifier) - set(uniquely_matched_ITNs.values())
                            if len(temptative_ITNs) == 1:
                                best_ITNs = list(temptative_ITNs)
                            else:
                                unique_SMILES = set([self.standardize_SMILES(infos_by_ITN[ITN][0], True) for ITN in temptative_ITNs])
                                if len(unique_SMILES) == 1:
                                    # if same structure, use the one with the smallest ITN
                                    best_ITNs = [str(min([int(ITN) for ITN in temptative_ITNs]))]
                                else:
                                    if len(temptative_ITNs) > 1:
                                        number_of_synonyms_found_in_ITN_info = []
                                        for ITN in temptative_ITNs:
                                            number_of_synonyms_found_in_ITN_info.extend(infos_by_ITN[ITN][5].count(il) * [ITN])
                    ...
                    if best_ITNs:
                        ...
                        SRS_results_by_identifier[identifier] = [infos_by_ITN[ITN] for ITN in best_ITNs]
    ...
    def get_molecules_from_SRS_batchmode(self, identifiers: list[str], mode: str, standardize: bool = True) -> list[Optional[Molecule]]:
        ...
        with self.query_molecule_cache_batchmode('srs', mode, identifiers, save_not_found=False) as (identifiers_to_search, indices_of_identifiers_to_search, results):
            ...
            for identifier_index, identifier in zip(indices_of_identifiers_to_search, identifiers_to_search):
                ...
                if len(f'{SRS_URL}/{mode}?{mode}List={urllib.parse.quote("|".join(this_chunk_identifiers))}&qualifier=exact') >= 2000:
            ...
            for chunk_identifier_indices, chunk_identifiers in zip(chunks_identifer_indices, chunks_identifiers, strict=True):
                search_response_text = self._resilient_request(f'{SRS_URL}/{mode}?{mode}List={urllib.parse.quote("|".join(chunk_identifiers))}&qualifier=exact',
                                                    rejected_status_codes=[404, 500], kwargs={'timeout':60})

                if search_response_text is not None:
                    infos_by_identifier = self._match_SRS_results_to_identifiers(chunk_identifiers, mode, standardize, json.loads(search_response_text))

                    for molecule_index, identifier in zip(chunk_identifier_indices, chunk_identifiers, strict=True):
                        if identifier in infos_by_identifier:
                            this_molecules = []
                            for SMILES, primary_names, synonyms, CAS, ITN, _ in infos_by_identifier[identifier]:
                                temp_synonyms = MoleculeResolver.filter_and_sort_synonyms(primary_names + synonyms)
                                this_molecules.append(Molecule(SMILES, temp_synonyms, CAS, ITN, mode, 'srs', identifier=identifier))
    ...
    @cache
    def _get_info_from_CIR(self, structure_identifier, representation, resolvers_to_use: Tuple[str], expected_number_of_results: Optional[int] = None) -> Optional[list[str]]:
        ...
        try:
            ...
            if response_values:
                if expected_number_of_results is not None:
                    if len(response_values) != expected_number_of_results:
                        raise RuntimeError('More than one iupac_name was found. This was unexpected.')
            ...
            if 'CIR_is_down' not in self._message_slugs_shown:
                ...
                warnings.warn('CIR seems to be down, to continue working this instance of MoleculeResolver will skip CIR.')
    ...
    def get_molecule_from_CIR(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        if required_formula is None:
            if mode == 'formula':
                required_formula = identifier
        
        MoleculeResolver._check_parameters(required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, services='cir', modes=mode)

        with self.query_molecule_cache('cir', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                resolvers_by_mode = {
                    'formula': None,
                    'name': ('name_by_cir',),
    ...
    def get_molecule_from_NIST(self, identifier: str, mode: str, required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:
        if required_formula is None:
            if mode == 'formula':
                required_formula = identifier

        MoleculeResolver._check_parameters(required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, services='nist', modes=mode)

        with self.query_molecule_cache('nist', mode, identifier) as (entry_available, molecules):
            if not entry_available:
                nist_modes = {
                    'formula': 'Formula',
                    'name': 'Name',
                ...
                NIST_Webbook_ID_regex = r'<a\s+href\s*=\s*"/cgi/cbook\.cgi\?ID=(.\d+).*">'
                ...
                def parse_molecule(temp_content):
                    ...
                    items = MoleculeResolver.parse_items_from_html(
                        ...
                        [
                            ...
                            ('text', fr'InChI: ({MoleculeResolver.InChI_regex_compiled.pattern[1:-1]})', [0, 1]),
                            ('text', f'CAS Registry Number: {MoleculeResolver.CAS_regex}', [0, 1]),
                            ('html', r'<li>\s*<strong>\s*Other\s+names:\s*</strong>(.*?)<\/li>', [0, 1])
                        ],
                        [0, 1]
                    )
                    if len(items) == 0:
                    ...
                    synonyms = [name.strip() for name in html.unescape(synonyms).split(';') if name.strip()]
                    ...
                    if MoleculeResolver.check_SMILES(SMILES, required_formula, required_charge, required_structure_type):
                        molecules.append(Molecule(SMILES, synonyms, CAS, additional_informationm, mode_used, 'nist', 1, identifier))
        
                if response_text is not None:

                    relevant_response_text = MoleculeResolver.normalize_html(response_text)
                    relevant_response_text = regex.findall('<main id="main">(.*)</main>', relevant_response_text)
                    ...
                    if not regex.search('<h1>.*(Not Found|no)\s*</h1>', relevant_response_text)  \
                        and not regex.search('<h1>.*No Matching Names Found\s*</h1>', relevant_response_text) \
                        and not regex.search('<h1>.*No Matching Species Found\s*</h1>', relevant_response_text)\
                        ...
                        if 'Search Results' not in relevant_response_text:
                            ...
                            for id in ids:
                                temp_result = self.get_molecule_from_NIST(id, 'cas', required_formula, required_charge, required_structure_type, standardize)
    ...
    def find_salt_molecules(self, identifiers: list[str], modes: list[str] = ['name'], required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, services_to_use: Optional[list[str]] = None, search_iupac_name: bool = False, interactive: bool = False, minimum_number_of_cross_checks: Optional[int] = 1, ignore_exceptions: bool = False) -> tuple[list, list[int]]:
        ...
        salt_info = self.find_single_molecule_cross_checked(identifiers, modes=modes, required_formula=required_formula, required_charge=required_charge, required_structure_type=required_structure_type, services_to_use=services_to_use, search_iupac_name=search_iupac_name, minimum_number_of_cross_checks=minimum_number_of_cross_checks, ignore_exceptions=ignore_exceptions)

        if salt_info[0] is None:
            salt_info = (given_SMILES, synonyms, CAS, 'given identifiers: ' + ', '.join(flattened_identifiers),  ', '.join(flattened_modes), 'manual', 1)
            if interactive:
                salt_info = self.find_single_molecule(identifiers, modes=modes, required_formula=required_formula, required_charge=required_charge, required_structure_type=required_structure_type, services_to_use=[], search_iupac_name=search_iupac_name, interactive=interactive, ignore_exceptions=ignore_exceptions)
        ...
        if salt_info[0] is not None:
            ...
            if MoleculeResolver.is_valid_SMILES(SMILES):
                if SMILES.count('.') > 0:
                    ...
                    ionic_SMILES_list = [MoleculeResolver.standardize_SMILES(smi, True) for smi in ionic_SMILES_list]
                ...
                for ionic_SMILES in ionic_SMILES_set:
                    ...
                    ionic_info = self.find_single_molecule_cross_checked(ionic_SMILES, modes='SMILES', required_charge=ionic_charge, required_structure_type='ion', services_to_use=services_to_use, search_iupac_name=search_iupac_name, minimum_number_of_cross_checks=minimum_number_of_cross_checks, ignore_exceptions=ignore_exceptions)

                    if ionic_info[0] is None:
                        ionic_info = (ionic_SMILES, [], [], f'given identifiers: {ionic_SMILES}', 'smiles', 'manual', 1)
                        if interactive:
                            ionic_info = self.find_single_molecule(ionic_SMILES, modes='SMILES', required_charge=ionic_charge, required_structure_type='ion', services_to_use=[], search_iupac_name=search_iupac_name, interactive=interactive, ignore_exceptions=ignore_exceptions)
        ...
        if len(all_molecules) < 3:
            ...
            for synonym in synonyms:
                ...
                cation_info = self.find_single_molecule_cross_checked(possible_cation_name, modes=['name'], required_charge='positive', required_structure_type='ion', services_to_use=services_to_use, search_iupac_name=search_iupac_name, minimum_number_of_cross_checks=minimum_number_of_cross_checks, ignore_exceptions=ignore_exceptions)

                if cation_info[0] is None:
                    if interactive:
                        cation_info = self.find_single_molecule(possible_cation_name, modes=['name'], required_charge='positive', required_structure_type='ion', services_to_use=[], search_iupac_name=search_iupac_name, interactive=interactive, ignore_exceptions=ignore_exceptions)

                possible_anion_name = synonym_parts[1:]
                
                anion_info = self.find_single_molecule_cross_checked(possible_anion_name, modes=['name'], required_charge='negative', required_structure_type='ion', services_to_use=services_to_use, search_iupac_name=search_iupac_name, minimum_number_of_cross_checks=minimum_number_of_cross_checks, ignore_exceptions=ignore_exceptions)

                if anion_info[0] is None:
                    if interactive:
                        anion_info = self.find_single_molecule(possible_anion_name, modes=['name'], required_charge='negative', required_structure_type='ion', services_to_use=[], search_iupac_name=search_iupac_name, interactive=interactive, ignore_exceptions=ignore_exceptions)
            ...
            if len(all_molecules) > 2:
                if len(all_molecules) != 3:
                    raise NotImplementedError('Functionality for salts with more than 2 ions has not been implemented.')

                cation_charge = Chem.rdmolops.GetFormalCharge(MoleculeResolver.get_from_SMILES(cation_info[0]))
                anion_charge = Chem.rdmolops.GetFormalCharge(MoleculeResolver.get_from_SMILES(anion_info[0]))

                if cation_charge >= abs(anion_charge):
                    if cation_charge % abs(anion_charge) == 0:
                        stoichometric_coefficients.append(1)
                        stoichometric_coefficients.append(abs(int(cation_charge / anion_charge)))
                    else:
                    ...
                    if abs(anion_charge) % cation_charge == 0:
                        stoichometric_coefficients.append(abs(int(anion_charge / cation_charge)))
                        stoichometric_coefficients.append(1)
                    else:
                        stoichometric_coefficients.append(abs(anion_charge))
                        stoichometric_coefficients.append(abs(cation_charge))
                ...
                if stoichometric_coefficients[0] != -1:  
                    ...
                    salt_info = (MoleculeResolver.standardize_SMILES('.'.join(temp_smiles), True), synonyms, list(CAS), 'from consisting ions', 'smiles', 'automatic', 1)
    ...
    def find_single_molecule(self, identifiers: list[str], modes: list[str] = ['name'], required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, services_to_use: Optional[list[str]] = None, standardize: bool = True, search_iupac_name: bool = False, interactive: bool = False, ignore_exceptions: bool = False) -> Optional[Molecule]:
        if services_to_use is None:
            services_to_use = MoleculeResolver._available_services

        flattened_identifiers, flattened_modes, synonyms, CAS, given_SMILES = MoleculeResolver._check_and_flatten_identifiers_and_modes(identifiers, modes)
        MoleculeResolver._check_parameters(services=services_to_use, required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, context='find_single')
        ...
        try:        
            for service in services_to_use:
                ...
                if service == 'cas_registry':
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_CAS_registry(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS = set(cmp.CAS) # overwrite CAS with data from the CAS registry
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_pubchem(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_CIR(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                additional_information = cmp.service
                                mode_used = mode
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_OPSIN(identifier, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                additional_information = cmp.service
                                mode_used = mode
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_ChEBI(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_SRS(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_CompTox(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_Chemeo(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_CTS(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
                    ...
                    for identifier, mode in zip(flattened_identifiers, flattened_modes):
                        if mode in MoleculeResolver._supported_modes_by_services[service]:
                            cmp = self.get_molecule_from_NIST(identifier, mode, required_formula, required_charge, required_structure_type, standardize)
                            if cmp is not None:
                                SMILES = cmp.SMILES
                                synonyms.extend(cmp.synonyms)
                                CAS.update(cmp.CAS)
            ...
            if SMILES is None:
                if given_SMILES is not None:
                    if MoleculeResolver.check_SMILES(given_SMILES, required_formula, required_charge, required_structure_type):
                        SMILES = given_SMILES
            ...
            if SMILES is None:
                if len(synonyms) > 0 and required_charge is not None:
                    ...
                    if required_charge != 'zero' and required_charge != 0:
                        for identifier, mode in zip(flattened_identifiers, flattened_modes):
                            molecules = self.get_molecule_for_ion_from_partial_pubchem_search(identifier, required_formula, required_charge, standardize)
                            if molecules is not None:
                                if len(molecules) > 0:
                                    mode_used = mode
                                    identifier_used = identifier
                                    current_service = 'pubchem'
                                    additional_information = 'get_SMILES_for_ion_from_partial_pubchem_search'
                                    SMILES = molecules[0][0].SMILES # get most likely candidate
                                    break

            if interactive and SMILES is None:
                return self.find_single_molecule_interactively(identifiers, modes, required_formula=required_formula, required_charge=required_charge, standardize=standardize)
        ...
        return Molecule(SMILES, synonyms, list(CAS), additional_information, mode_used, current_service, 1, identifier_used)

    def find_single_molecule_interactively(self, identifiers: list[str], modes: list[str] = ['name'], required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, standardize: bool = True) -> Optional[Molecule]:

        flattened_identifiers, flattened_modes, synonyms, CAS, given_SMILES = MoleculeResolver._check_and_flatten_identifiers_and_modes(identifiers, modes)
        MoleculeResolver._check_parameters(required_formulas=required_formula, required_charges=required_charge, required_structure_types=required_structure_type, context='find_single')
        ...
        while SMILES is None:
            ...
            try:
                if mode == 'smiles':
                    temptative_SMILES, temptative_synonyms, temptative_CAS, temptative_start_geometry_comment, mode_used = identifier, synonyms, CAS, 'manual geometry', 'interactive'
                else:
                    cmp = self.find_single_molecule([identifier], [mode], required_formula, required_charge, standardize=standardize)
                    temptative_SMILES, temptative_synonyms, temptative_CAS, temptative_start_geometry_comment, mode_used = (cmp.SMILES, cmp.synonyms, cmp.CAS, cmp.additional_information, cmp.mode) if cmp is not None else (None, [], [], '', '')
            except Exception:
                if mode == 'cid':
                    print('There was a problem retreiving the molecule from pubchem. Does it exist?')

            if MoleculeResolver.check_SMILES(temptative_SMILES, required_formula, required_charge, required_structure_type):
                temptative_synonyms = MoleculeResolver.filter_and_sort_synonyms(synonyms + temptative_synonyms)
                if len(temptative_synonyms) == 0:
                    possible_name = ''
                    while True:
                        possible_name = prompt_session.prompt('Input one name for the molecule', default=possible_name)
                ...
                if temp_mol is not None:
                    ...
                    print('Error: Image could not be gernerated from SMILES, this however does not mean always that the SMILES ist wrong.')
                    
                if yes_no_dialog(title='Confirmation', text='Is the molecule structure correct?').run():
                    ...
                    while True:
                        temptative_CAS = prompt_session.prompt('Input valid CAS or empty:', default=','.join(temptative_CAS))

                        if len(temptative_CAS) == 0 or all([MoleculeResolver.is_valid_CAS(x.strip()) for x in temptative_CAS.split(',')]):
    ...
    def find_single_molecule_cross_checked(self, identifiers: list[str], modes: list[str] = ['name'], required_formula: Optional[str] = None, required_charge: Optional[int] = None, required_structure_type: Optional[str] = None, services_to_use: Optional[list[str]] = None,  standardize: bool = True, search_iupac_name: bool = False, minimum_number_of_cross_checks: Optional[int] = 1, try_to_choose_best_structure: bool = True, ignore_exceptions: bool = False) -> Optional[Molecule] | list[Optional[Molecule]]:
        ...
        if not minimum_number_of_cross_checks <= len(services_to_use):
            raise ValueError('The minimum_number_of_cross_checks exceeds the number of services that are used.')

        molecules = []

        for service in services_to_use:
            molecule = self.find_single_molecule(identifiers=identifiers, modes=modes, required_formula=required_formula, required_charge=required_charge,
                                    required_structure_type=required_structure_type, services_to_use=[service], standardize=standardize,search_iupac_name=search_iupac_name,ignore_exceptions=ignore_exceptions)
        ...
        grouped_molecules = MoleculeResolver.group_molecules_by_structure(filtered_molecules, False)
        
        maximum_number_of_crosschecks_found = max([len(v) for v in grouped_molecules.values()])
    ...
    def find_multiple_molecules_parallelized(self, identifiers: list[str], modes: list[str], required_formulas: Optional[list[str]]=None, required_charges: Optional[list[int]] = None, required_structure_types: Optional[list[str]] = None, services_to_use: Optional[list[str]] = None, standardize: bool = True, search_iupac_name: bool = False, minimum_number_of_cross_checks: Optional[int] = 1, try_to_choose_best_structure: bool = True, progressbar: bool = True, max_workers: int = 5, ignore_exceptions: bool = True) -> list[Optional[Molecule] ]:
        ...
        MoleculeResolver._check_parameters(modes = modes, services = services_to_use, required_charges=required_charges, required_formulas=required_formulas,
                                required_structure_types=required_structure_types, context = 'find_multiple')
        ...
        if len(identifiers) > 100:
            ...
            services_asked_for_with_batch_capabilities = set(services_to_use).intersection(set(self._available_services_with_batch_capabilities))
            services_asked_for_with_batch_capabilities = sorted(list(services_asked_for_with_batch_capabilities))
            ...
            for service in services_asked_for_with_batch_capabilities:
                ...
                if not MoleculeResolver._is_list_of_list_of_str(identifiers): # Convert list[str] to list[list[str]] for usage in batchmode
                    identifiers = [[idf] for idf in identifiers]
                if not MoleculeResolver._is_list_of_list_of_str(modes):
                    modes = [[md] for md in modes]

                self.get_molecules_using_batchmode_from(identifiers, modes, service, progressbar=progressbar, ignore_exceptions=ignore_exceptions)

        def _find(generator):
            return list(tqdm(generator, total=len(identifiers), disable=not progressbar))

        args = []
        for identifier, mode, required_formula, required_charge, required_structure_type in zip(identifiers, modes, required_formulas, required_charges, required_structure_types):
            if minimum_number_of_cross_checks is None:
                args.append((identifier, mode, required_formula, required_charge, required_structure_type, services_to_use, standardize, search_iupac_name, False, ignore_exceptions))
            else:
                args.append((identifier, mode, required_formula, required_charge, required_structure_type, services_to_use, standardize, search_iupac_name, minimum_number_of_cross_checks, try_to_choose_best_structure, ignore_exceptions))
